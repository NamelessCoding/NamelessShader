...
for(int i = 0; i < 25; i++){
	...
	vec2 fin = (coords2*ires + coords)/ires;
    col += clamp(texture2D(colortex7, fin).w, 0.1, 1.0)*atrous_kernel_weights[i];
...
for(int i = 0; i < 25; i++){
	...
	vec2 fin = (coords2*ires + coords)/ires;
    col += clamp(1.0- texture2D(colortex11, fin).w, 0.0, 1.0)*atrous_kernel_weights[i];
...
if( mmm > 0.8 ){
  //  col = blurAM(iResolution, colortex6, 2., TexCoords*0.5).xyz*max(1.0-mmm,0.) + col;

///col = blurt3(TexCoords*0.5)*0.9 + col*(0.8);
col = texture2D(colortex6, TexCoords*0.5).xyz*0.1 + col;
//col = blurAM(iResolution, colortex6, 4., TexCoords*0.5).xyz*max(1.3-mmm, 0.) + col;
//col *= max(texture2D(colortex10, TexCoords*0.5).xyz,0.0)*2.;
}
...
//NOT MY CODE/////
 //float cutoff = 0.025;/
//col += (cutoff * 2. - col) * clamp(cutoff * 2. - col, 0., 1.) * (0.25 / cutoff) - cutoff;
//col = col*(0.5+6.2*col)/(0.06+col*(1.7+6.2*col));
///////////////////////////////
