...
void main() {
  ...
  velocity =
      exp(-length((ProjectedCoordinates - TexCoords) * Resolution * 0.01)) * 1.;
  ...
  if (ProjectedCoordinates.x > 1.0 || ProjectedCoordinates.x < 0.0 ||
    ...
    float mm =
        clamp(MN(clamp(length(ProjectedCoordinates - TexCoords) * 13.3, 0., 2.),
                 0.05, 0.2),
              0.2, 0.99);
    KeepColor = texture2D(colortex1, TexCoords * 0.5).xyz * (0.05) +
                texture2D(colortex7, ProjectedCoordinates.xy).xyz * 0.95;

    // KeepColor = mix(texture2D(colortex1, TexCoords).xyz,texture2D(colortex7,
    // ProjectedCoordinates.xy).xyz,finalWeigth);
    float avgLum = lum(texture2D(colortex1, TexCoords * 0.5).xyz) * 0.07 +
                   texture2D(colortex7, ProjectedCoordinates.xy).w * 0.93;

    float amb = clamp(min(texture2D(colortex10, TexCoords * 0.5).w,
                          texture2D(colortex7, ProjectedCoordinates.xy).w),
                      0.3, 1.0);

    data7 = vec4(KeepColor, amb);

    // variance = pow(lum(texture2D(colortex1, TexCoords*0.5).xyz) -
    // avgLum,2.)*0.07 + texture2D(colortex4, ProjectedCoordinates.xy).w*0.93;
    // gl_FragData[3] = vec4(texture2D(colortex3, TexCoords).xyz, variance);

    ambientO =
        ambientO * 0.1 + texture2D(colortex11, ProjectedCoordinates.xy).w * 0.9;
  }
  //}
