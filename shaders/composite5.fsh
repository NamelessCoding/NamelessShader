...
float wrp = exp(-(abs((1.0-lum(currrp)) - lum(nextrp))/(.75 + 0.15)));
float weigth = 1.0;
//if(check <= 1.0){
//weigth = wd*wp*spll;
//}else{
...
float mmm =  texture2D(colortex5, TexCoords*0.5).w;
  ...
  if(mmm != 20.5){
   gl_FragData[4] = vec4(blurAM(iResolution),texture2D(colortex4, TexCoords).w);
