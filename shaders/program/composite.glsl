...
vec3 renderpixel(vec3 p, vec3 d, inout uint r, inout float distat, inout vec3 ptp, inout vec3 firstcol, inout float spec,
 inout vec3 firstnorm, inout vec3 prevcol2, inout float distat2, inout vec3 normsec, inout vec3 bouncedcol, inout float ao){
...
vec3 direct = texelFetch(colortex13, ivec2(texcoord*iResolution*0.125  + vec2(rndf(r)*2.0-1.0, rndf(r)*2.0-1.0)*64. ), 0).xyz;;
float specular22 = 0.;
...
float probability =  clamp(specular22,0.1,0.99);//(specular22 < 0.1)?0.1:0.9;
//probability = 0.1;
//probability = 0.1;
...
vec3 specular4 = (D4*G4*F4)/max(4.*max(dot(ldir3,n),0.6)*max(dot(-wi,n),0.),0.001);
...
vec3 specular3 = (D3*G3*F3)/max(4.*max(dot(ldir2,n2),0.6)*max(dot(-wi2,n2),0.),0.001);
brdf3 = lerp(brdf3, specular3, 0.5);
//pdf = lerp(pdf, ggx_pdf(max(dot(reflect(wi,n), d),0.), rough), probability);
//brdf3 *= 1.0+(1.0*max(dot(d,n2),0.0));
//pdf = max(pdf, 0.0001);

if(ID2 == 58 || ID2 == 59 || ID2 == 60 || ID2 == 61 || ID2 == 62 || ID2 == 63 || ID2 == 64 || ID2 == 65 || ID2 == 66 || ID2 == 67 || ID2 == 68 || ID2 == 69 || ID2 == 70 || ID2 == 45
|| ID2 == 46 || ID2 == 47 || ID2 == 48 || ID2 == 49 || ID2 == 50 || ID2 == 51 || ID2 == 52 || ID2 == 53 || ID2 == 54 || ID2 == 55 || ID2 == 56 || ID2 == 300  || ID2 == 57 ){
col3 += tt2*c2*1.0*percent;
}
vec3 woo2 = quickShadow2(VoxelSpaceToSceneSpace(p2));
col3 += tt2*brdf3*15.*max(woo2,0.0)*vec3(0.9,0.7,0.5)*max(dot(ldir2,n2),0.0)*percent*f;
...
//col = vec3(probability);
//break;
 rough =max(1.0-specular22,0.1);// (max(1.0-specular22, 0.1)<0.1)?0.1:0.9;
rough = 0.1;
rough = rough*rough;
/*
if(ID == 250 ){
...
// specular2 = (D2*G2*F2)/max(4.*max(dot(ldir,n),0.6)*max(dot(-wi,n),0.),0.001);
//brdf2 = lerp(brdf2, specular2, probability);
//pdf = lerp(pdf, ggx_pdf(max(dot(reflect(wi,n), d),0.), rough), probability);
//brdf2 *= 1.0+(1.0*probability*max(dot(d,n),0.0));
//pdf = max(pdf, 0.0001);
...
col2 += tt*brdf2*3.*woo*vec3(0.9,0.7,0.5)*max(dot(ldir,n),0.0);
...
//accum += quickShadow3( VoxelSpaceToSceneSpace(WorldSpaceToVoxelSpace(cam2)));
//}
...
float shad = quickShadow3( VoxelSpaceToSceneSpace(WorldSpaceToVoxelSpace(cam2)));
if(shad < 0.05){
m = .45;
}
//accum += shad;
...
float ao = 0.;
		vec3 colorr = renderpixel(SceneSpaceToVoxelSpace(gbufferModelViewInverse[3].xyz), dir, r, distat, prevp, prevcol, spec, firstnorm, prevcol2, distat2,normsec, bouncedcol, ao);
