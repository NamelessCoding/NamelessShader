...
vec3 divideWith = texture2D(colortex4, (p+coords2)/iResolution).xyz;
ppp += lum(divideWith);
}
return ppp/9.;
}
...
//float mmm =  texture2D(colortex5, TexCoords*0.5).w;
   ...
   // if(mmm != 20.5){
    gl_FragData[0] = vec4(blurAM(iResolution),texture2D(colortex4, TexCoords).w);
      //  gl_FragData[1] = vec4(blurAM2(iResolution, colortex6, 2., TexCoords).xyz,texture2D(colortex4, TexCoords).w);
